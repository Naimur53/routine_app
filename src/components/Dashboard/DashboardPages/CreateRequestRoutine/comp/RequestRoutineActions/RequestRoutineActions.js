...
const RequestRoutineActions = ({ id, setData, data }) => {
    ...
    return (
        <div className='mt-10'>
            <div className='mb-10 flex flex-col md:flex-row gap-4  justify-between md:items-center'>
                ...
                </div>
                {
                    <div className='h-full w-[220px]'>
                        <TextField id="standard-basic" sx={{ width: "100%" }} label="Add message" disabled={postLoading} inputRef={adminMessage} defaultValue={data.adminMessage} variant="filled" />

                    </div>
                }
                <div>
                    {
                        postLoading ? <CircularProgress></CircularProgress> : <Button onClick={handleUploadStatus} disabled={data.status === status} variant="outlined">Update status</Button>
                    }
            ...
            <div className=''>
                {
                    data.requestId ? <div className='flex justify-center text-center pb-5'>
                        <div>
                            <h2 className='text-xl mb-2'>Routine Submitted</h2>
                            <p>
                                You can update Routine <Link className='text-dark-purple underline' to={`/update/${data.requestId}`}>from here</Link>
                            </p>
                        </div>

                    </div> : <CreateRoutine request={true} setRequestData={setData} requestId={data._id}></CreateRoutine>
                }
