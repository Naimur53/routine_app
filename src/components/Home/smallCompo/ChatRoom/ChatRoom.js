...
import { useAddMessageMutation, useGetMessagesWithRoutineIdQuery } from '../../../../ManageState/features/message/messageApi';
...
const ChatRoom = () => {
    ...
    const { data, isLoading, isError, isSuccess } = useGetMessagesWithRoutineIdQuery(routineId)
    const [addMessage, { isLoading: isAddMessageLoading, }] = useAddMessageMutation()
    const { data: messages, totalMessage, skip } = data || { messages: [], totalMessage: 0, skip: 0 }
    ...
    const onSubmit = async data => {
        ...
        addMessage({ user, routineId, img: watch('img'), message: data.message, date: Date().toString() })
        reset()
    }
    ...
    return (
        <>
            ...
            <div className='h-full flex flex-col bg-light-purple px-4 rounded-md '>
                <ChatBox skip={skip} routineId={routineId} totalMessage={totalMessage} messages={shortedByDate()} isLoading={isLoading} ></ChatBox>
                ...
                <Box sx={{ flexGrow: '0' }} className='rows-span    bg-primary  w-full   flex '>
                    ...
                    <form className='flex items-center  w-full mb-5' onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div>
                            {
                                imgLoading ? <button disabled={imgLoading || isLoading} className='bg-gray-100 text-gray-500 opacity-75  rounded-full p-2 inline-flex justify-center' type='submit'><SendIcon></SendIcon></button> : <button className='bg-white rounded-full p-2 inline-flex justify-center' type='submit'><SendIcon></SendIcon></button>
                            }
                        </div>
                    </form>
                </Box>
