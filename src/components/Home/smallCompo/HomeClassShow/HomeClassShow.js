...
const HomeClassShow = () => {
  ...
  const SingleTab = ({
    info: { day, totalClass, totalHours },
    onClick,
    i,
    value,
  }) => {
      ...
      <button className="cursor-pointer" onClick={() => handleChange(i)}>
        <div
          className={`flex flex-col lg:flex-row justify-center lg:justify-start items-center py-3 px-2 gap-4 rounded-2xl text-main-dark ${
            value === i && " active_ className_tab"
          }`}
        >
          <div
            className={`border-b lg:border-b-0 lg:border-r w-[80px] h-[50px] flex justify-center items-center  border-content `}
          >
            <h1 className="text-2xl  font-medium">{day.slice(0, 3)}</h1>
          </div>
          <div className="text-left">
            <h2 className="text-main-dark "> {totalClass} class</h2>
            <p className="text-content ">
              {" "}
              <span className="hidden lg:inline-block"> Total hours</span>{" "}
              {totalHours}{" "}
    ...
    <div className="">
      <Grid container spacing={2}>
        <Grid item xs={12} lg={6}>
          <Box className="flex lg:flex-col lg:custom_height overflow-y-auto">
            {week.map((single, i) => (
              <SingleTab
                value={value}
                key={single}
                ...
                component={SingleTab}
                info={{ day: single, totalClass: 10, totalHours: 5.4 }}
                {...a11yProps(i)}
              />
            ))}
          </Box>
