  ...
  const handleDelete = () => {
    if (!deleteLoading) {
      if (window.confirm("are you sure ?")) {
        ...
        axios
          .delete(`https://shielded-dusk-65695.herokuapp.com/routine/${_id}`)
          .then((res) => {
                ...
                <div className="flex gap-5">
                  <Tooltip title={department}>
                    <p className={`text-sm ${contentStyle}`}>
                      {textConversion(department, 9)}
                    </p>
                  </Tooltip>
                  <p className={`text-sm ${contentStyle}`}>{semester} sem</p>
                </div>
              </div>
            </div>
              ...
              <div className=" ">
                <Tooltip title={"Creator " + creator?.displayName}>
                  <h3 className="text-xs capitalize">
                    {textConversion(creator?.displayName, 25)}
                  </h3>
                </Tooltip>
                ...
                {updateAble ? (
                  <>
                    ...
                    <NavLink
                      to={
                        admin
                          ? `/dashboard/manageRoutine/update/${_id}`
                          : `/update/${_id}`
                      }
                    >
