...
const getRoutineDataFromLocalDbWithIndex = (index) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const { data: basicInfo, error: basicInfoError } = checkTypeString(
      data,
      basicInfoKey
    );
    ...
    const { data: userInfo, error: userInfoError } = checkTypeString(
      data.creator,
      userInfoKey
    );
    ...
    creatingData.totalUserUsing =
      typeof data.totalUserUsing !== "number" ? data.totalUserUsing : 0;
...
const getAllRoutinesFromLocalDb = () => {
  const allPromise = getDataFromLocalDb("routines")?.map((single, i) =>
    getRoutineDataFromLocalDbWithIndex(i)
  );
